...
import 'package:my_manga_editor/feature/manga/repository/exceptions.dart' as repo_exceptions;
...
class MangaRepository {
  ...
  final Map<String, String> _pageToMangaMap = {};
  ...
  Future<void> updateMangaName(String id, String name) async {
    try {
      if (name.isEmpty || name.length > 100) {
        throw repo_exceptions.ValidationException('Name must be 1-100 characters');
      }
  ...
  Future<void> updateStartPage(String id, MangaStartPage value) async {
    try {
      final userId = _authService.currentUser?.uid;
      if (userId == null) {
        throw repo_exceptions.AuthException();
  ...
  Future<void> deleteManga(String id) async {
    try {
      ...
      await _firebaseService.deleteManga(id);
  ...
  Future<String> createNewMangaPage(String mangaId) async {
    try {
      final userId = _authService.currentUser?.uid;
      if (userId == null) {
        throw repo_exceptions.AuthException();
      ...
      final existingPages = await _firebaseService.fetchMangaPages(mangaId);
      ...
      final pageId = await _firebaseService.createMangaPage(mangaId, cloudPage);
  ...
  Stream<MangaPage?> getMangaPageStream(String pageId) {
    try {
      ...
      if (mangaId != null) {
        return _firebaseService.watchMangaPageWithMangaId(mangaId, pageId).map((cloudPage) {
        ...
        logger.w('MangaId not found for pageId: $pageId. Use getMangaPageStreamWithMangaId instead.');
  ...
  Stream<MangaPage?> getMangaPageStreamWithMangaId(String mangaId, String pageId) {
    try {
      // Update the mapping
      _pageToMangaMap[pageId] = mangaId;
  ...
  Stream<List<String>> watchAllMangaPageIdList(String mangaId) {
    try {
      return _firebaseService.watchMangaPages(mangaId).map((cloudPages) {
        // Sort by pageIndex
        ...
        for (final page in sorted) {
          _pageToMangaMap[page.id] = mangaId;
        }

        return sorted.map((cp) => cp.id).toList();
      });
    } catch (e) {
  ...
  Future<void> reorderPages(String mangaId, List<String> pageIdList) async {
    try {
      final userId = _authService.currentUser?.uid;
      if (userId == null) {
        throw repo_exceptions.AuthException();
      ...
      for (int i = 0; i < pageIdList.length; i++) {
        batch.updatePage(mangaId, pageIdList[i], {'pageIndex': i});
      }
  ...
  /// Delete a manga page
  Future<void> deleteMangaPage(String pageId) async {
    try {
      final userId = _authService.currentUser?.uid;
      if (userId == null) {
        throw repo_exceptions.AuthException();
      ...
      final mangaId = _pageToMangaMap[pageId];
      if (mangaId == null) {
        throw repo_exceptions.NotFoundException('MangaPage', pageId);
      }

      await _firebaseService.deleteMangaPage(mangaId, pageId);
  ...
  Future<String> toMarkdown(String mangaId) async {
    try {
      // Fetch manga and pages
      final manga = await _firebaseService.fetchManga(mangaId);
      if (manga == null) {
        throw repo_exceptions.NotFoundException('Manga', mangaId);
      }

      final pages = await _firebaseService.fetchMangaPages(mangaId);
      ...
      if (manga.ideaMemo.isNotEmpty) {
        final ideaMemoOps = (manga.ideaMemo['ops'] as List?)?.cast<Map<String, dynamic>>() ?? [];
      ...
      for (int i = 0; i < pages.length; i++) {
        ...
        final memoOps = (page.memoDelta['ops'] as List?)?.cast<Map<String, dynamic>>() ?? [];
        ...
        final stageOps = (page.stageDirectionDelta['ops'] as List?)?.cast<Map<String, dynamic>>() ?? [];
        ...
        final dialoguesOps = (page.dialoguesDelta['ops'] as List?)?.cast<Map<String, dynamic>>() ?? [];
  ...
  Exception _handleFirebaseException(FirebaseException e) {
    if (e.code == 'permission-denied') {
      ...
      return repo_exceptions.StorageException(e.message ?? 'Storage error', code: e.code);
    }
  }
...
extension CloudMangaConversion on CloudManga {
  Manga toManga(DeltaCache cache) {
    ...
    final ideaMemoOps = (ideaMemo['ops'] as List?)?.cast<Map<String, dynamic>>() ?? [];
    final delta = Delta.fromJson(ideaMemoOps);
...
extension CloudMangaPageConversion on CloudMangaPage {
  MangaPage toMangaPage(DeltaCache cache) {
    ...
    final memoOps = (memoDelta['ops'] as List?)?.cast<Map<String, dynamic>>() ?? [];
    final memoDeltaObj = Delta.fromJson(memoOps);
    final memoDeltaId = cache.storeDelta(memoDeltaObj);

    final stageOps = (stageDirectionDelta['ops'] as List?)?.cast<Map<String, dynamic>>() ?? [];
    final stageDeltaObj = Delta.fromJson(stageOps);
    final stageDeltaId = cache.storeDelta(stageDeltaObj);

    final dialoguesOps = (dialoguesDelta['ops'] as List?)?.cast<Map<String, dynamic>>() ?? [];
...
extension MangaPageToCloudConversion on MangaPage {
  CloudMangaPage toCloudMangaPage(String mangaId, int pageIndex, DeltaCache cache) {
    ...
    if (memoDeltaObj == null || stageDeltaObj == null || dialoguesDeltaObj == null) {
